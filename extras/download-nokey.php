...
$dldate = date("m.d.y");
$filename = $_SERVER['DOCUMENT_ROOT'] . "/assessmentsFile/rubrics/". $stuid ."_s".$semester."_full.php";
$filename2 = $_SERVER['DOCUMENT_ROOT'] . "/assessmentsFile/rubrics/". $stuid ."_s".$semester."_full.pdf";
...
while($subjrow = mysql_fetch_array($subjresult)){
	...
	while($headingrow = mysql_fetch_array($headingresult)){
		$skillsql = "SELECT * FROM elem_skills WHERE headingid = '".$headingrow['headingid']."' AND subjectid = '$subjectid' AND grade = '$skillgrade' AND semester <= '".$semester."' ORDER BY skill_sort ASC";
...
//header('Content-Disposition: attachment; filename="'.$_GET['stuid'].'.pdf"');
